...
prints instructions the other agent can follow to connect. Works with Claude,
Codex, Gemini, Cursor, or any agent that can make HTTP requests. The remote agent
...
You're sitting in a coding agent with a browser running. You also have another AI agent
open (Claude, Codex, Gemini, Cursor, or another HTTP-capable agent). You want it to be
...
copy-paste ceremony and write the credentials directly to
...
Replace `TARGET_HOST` with the value from Step 2 (`claude`, `codex`, `gemini`, or `cursor`).
...
Gemini CLI can run the instruction block's curl commands directly. When using
`--local gemini`, credentials are written to
`~/.gemini/skills/gstack/browse-remote.json`.
