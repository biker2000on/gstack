...
import { claude as claudeHost, getExternalHosts, ALL_HOST_CONFIGS } from '../hosts/index';
...
for (const { tmpl, output } of TEMPLATES) {
  ...
  if (!fs.existsSync(outPath)) {
    ...
    if (claudeHost.generation.skipSkills?.includes(skillDir)) {
      console.log(`  -  ${tmpl.padEnd(30)} — external-host-only skill`);
      continue;
    }
    hasErrors = true;
